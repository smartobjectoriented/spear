...
from standard_vector_index import LocalStandardEmbedder, cosine_similarity
...
def rebuild_lexical_index(store: StandardStore, standard_id: str, revision: str, *,
    ...
    for unit in units:
        tokens = tokenize(" ".join((unit.section or "", *unit.heading_path, unit.text)))
...
class StandardRetrieval:
    ...
    def search_response(self, standard_id: str, revision: str, query: str, *,
        ...
        if mode != "lexical":
            ...
            except (FileNotFoundError, StandardStoreError, RuntimeError):
        ...
        for rank, (score, source_id, referenced_from) in enumerate(
                [(score, sid, None) for score, sid in primary] + expanded, 1):
            unit = units[source_id]
            ...
            reason = ("cross_reference" if referenced_from else "exact_section" if exact
